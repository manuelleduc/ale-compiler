...
public class DslType extends XsemanticsRuntimeSystem {
  ...
  public final static String CLASSTYPEAAAA = "dsl.xtext.ClassTypeAAAA";
  ...
  protected Result<TypeSystem> classTypeImpl(final RuleEnvironment G, final RuleApplicationTrace _trace_, final AleClass aleClass) throws RuleFailedException {
    try {
    	final RuleApplicationTrace _subtrace_ = newTrace(_trace_);
    	final Result<TypeSystem> _result_ = applyRuleClassTypeAAAA(G, _subtrace_, aleClass);
    	addToTrace(_trace_, new Provider<Object>() {
    		public Object get() {
    			return ruleName("ClassTypeAAAA") + stringRepForEnv(G) + " |> " + stringRep(aleClass) + " : " + stringRep(_result_.getFirst());
    		}
    	});
    	addAsSubtrace(_trace_, _subtrace_);
    	return _result_;
    } catch (Exception e_applyRuleClassTypeAAAA) {
    	classTypeThrowException(ruleName("ClassTypeAAAA") + stringRepForEnv(G) + " |> " + stringRep(aleClass) + " : " + "ClassTypeT",
    		CLASSTYPEAAAA,
    		e_applyRuleClassTypeAAAA, aleClass, new ErrorInformation[] {new ErrorInformation(aleClass)});
    	return null;
    }
  }
  
  protected Result<TypeSystem> applyRuleClassTypeAAAA(final RuleEnvironment G, final RuleApplicationTrace _trace_, final AleClass aleClass) throws RuleFailedException {
    
    return new Result<TypeSystem>(_applyRuleClassTypeAAAA_1(G, aleClass));
  }
  
  private ClassTypeT _applyRuleClassTypeAAAA_1(final RuleEnvironment G, final AleClass aleClass) throws RuleFailedException {
